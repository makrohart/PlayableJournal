...
//                auto nativeValue = static_cast<Type*>(pNative)->invoke(&Type::Getter);                                                 \
...
//                static_cast<Type*>(pNative)->invoke(&Type::Setter, nativeValue);                                                         \
//            },                                                                                                            \
//        },
//
...
//	            v8::Local<v8::External> native = v8::Local<v8::External>::Cast(self->GetInternalField(0));    \
//	            void* pNative = native->Value();                                                              \
//                auto function = std::bind(&Type::invoke<decltype(&Type::Method) >, static_cast<Type*>(pNative), &Type::Method                          \
...
namespace playable
{
    struct Playable
    {
        static PlayableType& type(const std::string& name)
        {
            return *(new PlayableType(name));
        }
    };
}